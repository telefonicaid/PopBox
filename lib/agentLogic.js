...
function httpSubscribeQueueAux(reliable, req, res) {
  ...
  subscribeQueue({ id: req.param('id'), prefix: req.prefix, sender: sender, reliable: reliable});
...
function ackTransaction(req, res) {
  var transId = req.param('id_trans'),
      prefix = req.prefix,
      queue = req.body.queue;
  if (queue) {
    dataSrv.ackTransaction(transId, queue, function (err) {
      if (err) {
        logger.info('ackTransaction', [String(err), 500, req.info]);
        res.send(500, {errors: [String(err)]});
    ...
    logger.info('ackTransaction', [
      {errors: ['missing queue']},
      400,
      req.info
    ]);
    res.send(400, {errors: ['missing queue']});
  }
}
