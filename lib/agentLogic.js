...
function subscribeQueue(queueId, appPrefix, sender) {
  'use strict';
  var maxMsgs = 1;
...
function httpSubscribeQueue(req, res) {
  'use strict';
  var clientClosed = false;
  ...
  sender.onClose = function(cb) {
    req.connection.addListener('close', cb);
  }
  ...
  subscribeQueue(req.param('id'), req.prefix, sender);
...
function ioSubscribeQueue(socket, queueId) {
  ...
  sender.onClose = function(cb) {
    socket.addListener('disconnect', cb);
  }
  ...
  subscribeQueue(queueId, 'UNSEC:', sender);
}
  ...
  dataSrv.getSecHash(appPrefix, req.param('id'), function (err, value) {
    ...
    if (err) {
      ...
      if (digest === value) {
        ...
        res.set('Content-Type', 'text/plain');
        res.set('WWW-Authenticate', 'Basic realm="PopBox"');
        res.send(401, 'Unauthorized ' + username);
      }
    }
    else {
      res.set('Content-Type', 'text/plain');
...
exports.httpSubscribeQueue = httpSubscribeQueue;
