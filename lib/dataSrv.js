...
var getQueue = function(appPrefix, queueId, callback) {
  ...
  db.lrange(fullQueueIdH, 0, maxMessages, function onHRange(err, hQueue) {
    db.lrange(fullQueueIdL, 0, maxMessages, function onLRange(err, lQueue) {
      dbCluster.free(db);
      db.get(config.dbKeyQueuePrefix + appPrefix + queueId + ':lastPopDate', function(err, lastPopDate) {
        if (err) {
          lastPopDate = null;
        }
        db.get(config.dbKeyQueuePrefix + appPrefix + queueId + ':blocked', function(err, blocked) {

          blocked = blocked || false; //null is returned when blocked has not been set
...
var setBlockedQueue = function(appPrefix, queueId, blocked, cb) {
  ...
  dbCluster.getOwnDb(queueId, function(err, db) {
    if (err) {
      ...
      db.set(config.dbKeyQueuePrefix + appPrefix + queueId + ':blocked', blocked, cb);
    }
  });
}
