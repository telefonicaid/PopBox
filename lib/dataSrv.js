...
var setSecHash = function(appPrefix, queueId, user, passwd, callback) {
  ...
  token = new Buffer(user + ':' + passwd).toString('base64');
  shasum.update(token);
  digest = shasum.digest('base64');
  helper.createKey(db, appPrefix + queueId, digest, callback);
};
...
var popNotification = function(options, callback) {
  //pop the queu  (LRANGE)
  ...
  var db = options.db,
      ...
      firstElem = options.firstElem,
      reliable =  options.reliable,
      fullQueueIdH = config.dbKeyQueuePrefix + 'H:' + appPrefix +
        ...
        db.lrange(fullQueueIdL, 0, restElems - 1,
          ...
          if (errL && firstElem[0] !== fullQueueIdL) {
            ...
            if (dataH) {
              ...
              getPopData({data:dataH, queue: queue, reliable: reliable}, callback);
            } else {
...
var blockingPop = function(options, callback) {
  var queue = options.queue,
      queueId = queue.id,
      appPrefix = options.prefix,
      maxElems = options.limit,
      timeout = options.timeout,
      reliable = options.reliable,
      fullQueueIdH = config.dbKeyQueuePrefix + 'H:' + appPrefix + queueId,
      fullQueueIdL = config.dbKeyQueuePrefix + 'L:' + appPrefix + queueId,
      firstElem = null;
  ...
  function blockingPopAux(db) {
    db.hset(config.dbKeyQueuePrefix + appPrefix + queueId + ':userInfo', 'lastPopDate',
        popDate, function() {
        });
    //Do the blocking part (over the two lists)
    db.blpop(fullQueueIdH, fullQueueIdL, timeout,
        function onPopData(err, data) {
          if (err) {
            ...
            if (! data) {
              ...
              firstElem = data;
              if (maxElems > 1) {
                popNotification({db: db, prefix: appPrefix, queue: queue, maxElems: maxElems - 1, firstElem: firstElem,
                    reliable: reliable }, function onPop(err, clean_data) {
                ...
                dbCluster.free(db);
                //just first_elem
                getPopData({ data: [firstElem[1]], queue: queue, reliable: reliable}, callback);
              }
            }
          }
        });
...
function getPopData(options, callback) {
  var
      dataH = options.data,
      queue = options.queue,
      reliable = options.reliable,
  ...
  retrieveData(queue, dataH, function onData(err, payloadWithNulls) {
    if (err) {
      ...
      newStateBatch = cleanData.map(function prepareStateBatch(elem) {
        ...
        dbTr = dbCluster.getDb(elem.extTransactionId);
        return helper.hsetHashParallel(dbTr, queue, transactionId, ':state', 'Blocked');
      });
...
var getTransaction = function(extTransactionId, state, summary, callback) {
  ...
  //check params
  if (state !== 'All' && state !== 'Pending' && state !== 'Delivered'  && state !== 'Blocked') {
    //Wrong state
...
var getQueue = function(appPrefix, queueId, callback) {
  ...
  db.lrange(fullQueueIdH, 0, maxMessages, function onHRange(err, hQueue) {
    db.lrange(fullQueueIdL, 0, maxMessages, function onLRange(err, lQueue) {
      dbCluster.free(db);
      db.hgetall(config.dbKeyQueuePrefix + appPrefix + queueId + ':userInfo', function(err, userInfo) {
        var lastPopDate, blocked;

        if (err) {
          lastPopDate = null;
          blocked = false;
        } else {
          lastPopDate = userInfo.lastPopDate;
          blocked = (userInfo.blocked > 0) ? true : false;
        }

        if (callback) {
          callback(err, hQueue, lQueue, lastPopDate, blocked);
        }
      });
    });
  });
...
var ackTransaction = function ackTransaction(extTransactionId, queue, cb) {
  ...
  helper.exists(dbTr, state, function (errE, value) {
    if (errE) {
      return cb(errE);
    }
    else if (!value) {
      return cb(extTransactionId+'/' + queue + ' does not exist');
    }
    else {
      helper.hsetHashParallel(dbTr, {id: queue}, transactionId, ':state', 'Delivered')(cb);
    }
  });
};
var getMessageState = function getMessageState(extTransactionId, queue, cb) {
  var dbTr = dbCluster.getDb(extTransactionId),
      transactionId = config.dbKeyTransPrefix + extTransactionId,
      state = transactionId + ':state';
  dbTr.hget(state, queue, function (errE, value) {
    if (errE) {
      return cb(errE);
    }
    else if (!value) {
 ...
 * @param cb
 */
exports.setBlockedQueue = setBlockedQueue;
exports.ackTransaction = ackTransaction;
