    ...
    db.lrange(fullQueueIdL, 0, maxMessages, function onLRange(err, lQueue) {
      ...
      db.hgetall(config.dbKeyQueuePrefix + appPrefix + queueId + ':userInfo', function(err, userInfo) {
        ...
        if (err) {
          ...
          blocked = userInfo.blocked || false;   //null is returned when blocked has not been set

          //Redis stores bool values as strings
...
var setBlockedQueue = function(appPrefix, queueId, blocked, cb) {
  ...
  dbCluster.getOwnDb(queueId, function(err, db) {
    if (err) {
      ...
      db.hset(config.dbKeyQueuePrefix + appPrefix + queueId + ':userInfo', 'blocked', blocked, cb);
    }
  });
}
