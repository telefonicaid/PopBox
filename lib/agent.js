...
var start = function start(done) {
  ...
  if (cluster.isMaster && numCPUs !== 0) {
    ...
    if (config.enableSecure === true || config.enableSecure === 'true' ||
      ...
      appSec.use(express.basicAuth(logic.checkPerm);
...
var setRoutes = function setRoutes(app) {
  ...
  // Multitenant URLS
  app.get('/org/:id_org/trans/:id_trans', logic.transMeta);
  app.put('/org/:id_org/trans/:id_trans', logic.putTransMeta);
  app.post('/org/:id_org/trans', logic.postTransDelayed);

  app.post('/queue', checkSecure, logic.postQueue);
  app.post('/queue/:id/pop', checkSecure, logic.popQueue);
  app.get('/queue/:id/peek', checkSecure, logic.peekQueue);
  app.post('/queue/:id/subscribe', checkSecure, logic.httpSubscribeQueue);
  app.get('/queue/:id', checkSecure, logic.getQueue);
  app.get('/org/:id_org/queue/:id', checkSecure, logic.getQueue);
  app.post('/org/:id_org/queue/:id/pop', checkSecure, logic.popQueue);
  app.get('/org/:id_org/queue/:id/peek', checkSecure, logic.peekQueue);
  app.post('/org/:id_org/queue/:id/subscribe', checkSecure, logic.httpSubscribeQueue);
  ...
  app.post('/org/:id_org/group', groups.createGroup);
  app.get('/org/:id_org/group/:groupName', groups.getGroup);
  app.del('/org/:id_org/group/:groupName', groups.deleteGroup);
}

var checkSecure = function(req, res, next){
  if (req.secure){
    logic.checkPerm(req, res, next);
  }
  else {
    next();
  }
