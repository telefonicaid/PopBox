...
var setRoutes = function setRoutes(server) {
    var prefix = prefixer.prefixer(server.prefix);
    ...
    server.get('/', prefix, deployInfo.showDeployInfo);
    server.del('/trans/:id_trans', prefix, logic.deleteTrans);
    server.get('/trans/:id_trans', prefix, logic.transMeta);
    server.put('/trans/:id_trans', prefix, logic.putTransMeta);
    server.post('/trans', prefix,
        groups.addQueuesFromGroup,
        logic.postTransDelayed);
    server.get('/trans/:id_trans/state/:state?', prefix, logic.transState);

    // Multitenant URLS
    server.get('/org/:id_org/trans/:id_trans', prefix, logic.transMeta);
    server.put('/org/:id_org/trans/:id_trans', prefix, logic.putTransMeta);
    server.post('/org/:id_org/trans', prefix, logic.postTransDelayed);

    if(server.isSecure) {
        server.post('/queue', prefix, logic.postQueue);
        server.post('/queue/:id/pop', prefix, logic.checkPerm, logic.popQueue);
        server.get('/queue/:id/peek', prefix, logic.checkPerm, logic.peekQueue);
        server.post('/queue/:id/subscribe', prefix, logic.checkPerm, logic.subscribeQueue);
        server.get('/queue/:id', prefix, logic.checkPerm, logic.getQueue);
        server.get('/org/:id_org/queue/:id', prefix, logic.checkPerm, logic.getQueue);
        server.post('/org/:id_org/queue/:id/pop', prefix, logic.checkPerm, logic.popQueue);
        server.get('/org/:id_org/queue/:id/peek', prefix, logic.checkPerm, logic.peekQueue);
        server.post('/org/:id_org/queue/:id/subscribe', prefix, logic.checkPerm, logic.subscribeQueue);

    }
    else {
        server.post('/queue/:id/pop', prefix, logic.popQueue);
        server.get('/queue/:id/peek', prefix, logic.peekQueue);
        server.post('/queue/:id/subscribe', prefix, logic.subscribeQueue);
        server.get('/queue/:id', prefix, logic.getQueue);
        server.get('/org/:id_org/queue/:id', prefix, logic.getQueue);
        server.post('/org/:id_org/queue/:id/pop', prefix, logic.popQueue);
        server.get('/org/:id_org/queue/:id/peek', prefix, logic.peekQueue);
        server.post('/org/:id_org/queue/:id/subscribe', prefix, logic.subscribeQueue);
    }

    //Group management URLs
    server.post('/group', prefix, groups.createGroup);
    server.get('/group/:groupName', prefix, groups.getGroup);
    server.del('/group/:groupName', prefix, groups.deleteGroup);

    server.post('/org/:id_org/group', prefix, groups.createGroup);
    server.get('/org/:id_org/group/:groupName', prefix, groups.getGroup);
    server.del('/org/:id_org/group/:groupName', prefix, groups.deleteGroup);
}
