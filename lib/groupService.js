...
function createGroupInRedis(groupBody, callback) {
  var db = dbCluster.getDb(groupBody.name),
      redisActions = [];
...
function getGroup(req, res) {
  var db = dbCluster.getDb(req.params.groupName);
...
function getElementsHandler(name, callback) {
  var db = dbCluster.getDb(name);
  db.smembers(name, callback);
}

function addQueuesFromGroup(req, res, next) {
  if (req.body && req.body.groups) {
    async.map(req.body.groups, getElementsHandler, function (error, results) {
      if (req.body.queue) {
...
function modifyGroup(req, res) {
  ...
  console.log('modifyGroup', req.params.groupName);

  var errors = [];
  var groupName = req.params.groupName;
  var db = dbCluster.getDb(groupName);
  var redisActions = [];
