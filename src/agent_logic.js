...
function getQueue(req, res) {
  ...
  dataSrv.getQueue(prefix, queueId, function onGetQueue(err, hQ, lQ, lastPop) {
    ...
    if (err) {
      ...
      var filterTrans = function (item, callback) {
        helper.deleteOnExpired(item, queueId, function (res) {
          callback(res);
        });
      };
      async.parallel([
        function (cb) {
          async.filter(hQ, filterTrans, function (res) {
            cb(null, res);
          });
        },
        function (cb) {
          async.filter(lQ, filterTrans, function (res) {
            cb(null, res);
          });
        }
      ], function (err, result) {
