    ...
    agent.start(function () {
      utils.cleanBBDD(function () {
        ...
        for (var i = 0; i < N_TRANS; i++) {
          ...
          var trans = utils.createTransaction(MESSAGE_INDEX + i, 'H', [
            { 'id': QUEUE_NAME }
          ]);
    ...
    agent.start(function () {
      utils.cleanBBDD(function () {
        ...
        for (var i = 0; i < N_TRANS; i++) {
          ...
          var trans = utils.createTransaction(MESSAGE_INDEX + i, 'L', [
            { 'id': QUEUE_NAME }
          ]);
          utils.pushTransaction(trans, function (err, response, data) {
  ...
  before(function (done) {
    ...
    agent.start(function () {
      utils.cleanBBDD(function () {
        ...
        for (var i = 0; i < N_TRANS; i++) {
          var trans = utils.createTransaction(MESSAGE_INDEX + i, (i % 2 === 0) ? 'H' : 'L', [
            { 'id': QUEUE_NAME }
          ]);
