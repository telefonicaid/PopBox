...
var doPop = function (nPops, queue){
    ...
    var timeOut = randomize(config.randomTest.minTimeOut, config.randomTest.maxTimeOut);
...
var doForever = function () {
    ...
    if (pushOrPop < 5) {
        ...
        var payloadLength = randomize(config.randomTest.minPayloadLength, config.randomTest.minPayloadLength);
        var timesPush = randomize(1, config.randomTest.maxTimesPush);

        console.log('Pushing in ' + numQueues + ' queues with provisions of ' + payloadLength + ' bytes ' + timesPush + ' times');
